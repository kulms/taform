...
//if(isset($_SESSION['slogin']) && !empty($_SESSION['slogin'])) { 
	...
	$std_tmpfilename = "ot_approval_group.xlsx";
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
        ...
        $dept_id = $objWorksheet->getCell('A'.$row)->getValue();
		$emp_id = $objWorksheet->getCell('B'.$row)->getValue();
		$create_by = $objWorksheet->getCell('C'.$row)->getValue();
		$create_date = $objWorksheet->getCell('D'.$row)->getValue();
		$lupdate_by = $objWorksheet->getCell('E'.$row)->getValue();
		$lupdate_date = $objWorksheet->getCell('F'.$row)->getValue();
