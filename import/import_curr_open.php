...
//if(isset($_SESSION['slogin']) && !empty($_SESSION['slogin'])) { 
	...
	$std_tmpfilename = "sc_curr_open.xlsx";
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
        ...
        $curr_id = $objWorksheet->getCell('B'.$row)->getValue();
        $year_id = $objWorksheet->getCell('C'.$row)->getValue();
        $curr_open = $objWorksheet->getCell('D'.$row)->getValue();
        $curr_status_id = $objWorksheet->getCell('E'.$row)->getValue();        
        
        $sql = "INSERT INTO sc_curr_open (
			curr_id,            
			year_id,			
            curr_open,
            curr_status_id,						
