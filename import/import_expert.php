...
//if(isset($_SESSION['slogin']) && !empty($_SESSION['slogin'])) { 
	...
	$std_tmpfilename = "sc_expert.xlsx";
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
        ...
        $expert_title = $objWorksheet->getCell('B'.$row)->getValue();
		$expert_fname = $objWorksheet->getCell('C'.$row)->getValue();
        $expert_mname = $objWorksheet->getCell('D'.$row)->getValue();		
        $expert_lname = $objWorksheet->getCell('E'.$row)->getValue();
		$expert_degree = $objWorksheet->getCell('F'.$row)->getValue();
        
        $sql = "INSERT INTO sc_expert (
			expert_title,            
			expert_fname,			
            expert_mname,
			expert_lname,
