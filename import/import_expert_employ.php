...
//if(isset($_SESSION['slogin']) && !empty($_SESSION['slogin'])) { 
	...
	$std_tmpfilename = "sc_expert_employ.xlsx";
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
        ...
        $year_id = $objWorksheet->getCell('B'.$row)->getValue();
		$expert_id = $objWorksheet->getCell('C'.$row)->getValue();
        $dept_id = $objWorksheet->getCell('D'.$row)->getValue();		
        $budget_id = $objWorksheet->getCell('E'.$row)->getValue();
		$startdate = $objWorksheet->getCell('F'.$row)->getValue();
		$enddate = $objWorksheet->getCell('G'.$row)->getValue();
        
        $sql = "INSERT INTO sc_expert_employ (
			year_id,            
			expert_id,			
            dept_id,
			budget_id,
