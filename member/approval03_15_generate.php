<?php
    ...
    $app_id = $_GET["appid"];
    $sql = "SELECT a.*, y.year_name, d.dept_name, d.dept_head, d.dept_position, appt.app_type_name, appt.app_type_report, m.firstname, m.lastname  
            FROM approval a
    ...
    // $app_date = DateThai(date("Y-m-d"));

    $filename = "approval_form_".$today.".pdf";

    // $dept_position = $row["dept_position"];
    $dept_position = "คณบดี";

    $dept_head = $row["dept_head"];
    $dept_position2 = $row["dept_position"];

    $app_type_name = $row["app_type_name"];
    $app_type_report = $row["app_type_report"];
    $app_head_position = $row["app_head_position"];
    ...
    if(countRow($app_id,$conn)>12){
        ...
        $content2 .= ''.$app_type_report.' วงเงินไม่เกิน '.number_format($row["budget"],2).' บาท ('.num2wordsThai(substr($row["budget"],0,strlen($row["budget"])-3)).'บาทถ้วน)<br><br>';
        // $content2 .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;จึงเรียนมาเพื่อโปรดพิจารณา หากเห็นชอบโปรดเสนอประธานอนุมัติหลักการต่อไป<br><br>';
        $content2 .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;จึงเรียนมาเพื่อโปรดพิจารณา<br><br>';
        ...
        <tr>  
            ...
            <br><br><br> ลงนาม.................................... <br> ('.$dept_head.') <br> ตำแหน่ง '.$dept_position2.'
            </td>        
        </tr>
        <tr>
            <td>&nbsp;</td>
        ...
        <tr>  
            <td>            
                ...
                 อนุมัติ <br><br><br> ('.$asso_dean.') <br> ตำแหน่ง '.$asso_dean_pos.'<br>&nbsp;'.$asso_dean_pos2.' <br> วันที่....................................... 
            </td>
            <td></td>
            <td>            
            จองเงินเลขที่ ....................................... <br>&nbsp; หมวด ค่าตอบแทน'.$app_type_name.' <br>&nbsp; หน่วยงาน  '.$app_type_report.' <br> &nbsp;จำนวนเงิน ....................................... <br> จนท. การเงิน'.$app_type_report.' <br>           
            วันที่..........................&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </td>        
        </tr> 
        <table>       
        ...
        // $content .= '
        // <h2 align="center">แบบขออนุมัติหลักการปฏิบัติงานนอกเวลาราชการ</h2>
        // <h3 align="center">('.$app_type_name.')</h3>
        // <table border="0" cellspacing="0" cellpadding="3">  
        // <tr>  
        //     <td width="12%" class="th-left-color"><b>ส่วนงาน</b></td>
