<?php
    ...
    $filename = "otdata_work_form_".$today.".pdf";
	...
	// 	return $contents;
    // }
    function getFirstRow($id,$year_id,$otdata_month,$conn){
        if($_SESSION["deptid"]=='99'){
            $sql = "SELECT DISTINCT otdata_time_cal.*, employees.titlename, employees.firstname, employees.lastname, 
                    ...
                    ORDER BY CONVERT(dept_name USING tis620), otdata_time_cal.ot_date,
            ...
            $sql = "SELECT DISTINCT otdata_time_cal.*, employees.titlename, employees.firstname, employees.lastname, 
                    ...
                    ORDER BY otdata_time_cal.ot_date,
                    CONVERT(employees.firstname USING tis620), CONVERT(employees.lastname USING tis620)
                    LIMIT 1;";
        }
        $query = $conn->query($sql);
    ...
    function getLastRow($id,$year_id,$otdata_month,$conn){
        if($_SESSION["deptid"]=='99'){
            $sql = "SELECT DISTINCT otdata_time_cal.*, employees.titlename, employees.firstname, employees.lastname, 
                    department.dept_name AS dept_name, otrate.otrate_name, otrate.is_holiday
                    FROM otdata_time_cal
                    ...
                    LEFT JOIN approval_group ON approval_group.emp_id=otdata_time_cal.emp_id
                    WHERE otdata_time_cal.otdata_id =".$id." AND otdata_time_cal.ot_amount > 0                                   
                    ORDER BY otdata_time_cal.ot_date DESC                    
                    LIMIT 1;"; 
        }else{
            $sql = "SELECT DISTINCT otdata_time_cal.*, employees.titlename, employees.firstname, employees.lastname, 
                    department.dept_name AS dept_name, otrate.otrate_name, otrate.is_holiday                    
    ...
    function generateRow($id,$year_id,$otdata_month,$conn){
        ...
        while($row = $query->fetch_assoc()){
            ...
            if($ot_date_chk==""){
                $ot_date_chk=$row['ot_date'];
            }
            
            if($row['ot_date']!=$ot_date_chk){
                $i=1;
            }
            ...
            if($timein[0]>=16){
                ...
                <tr>                
                    ...
                    <td width="25%">'.$row["titlename"].$row["firstname"].' '.$row["lastname"].'</td>                
                    <td width="12%" align="center">'.DateShortThai($row['ot_date']).'</td>                    
                    <td width="12%" align="center">'.substr($row["ot1_form_in"],0,5).'</td>
                    <td width="12%" align="center">'.substr($row["ot2_form_out"],0,5).'</td>
                    <td width="18%">&nbsp;</td>		
                    <td width="12%">&nbsp;</td>		
                </tr>
                ';
    ...
    $content .= '
    <h2 align="center">บัญชีแสดงการมาทำงานนอกเวลาราชการ</h2>
    <h4 align="center">ส่วนราชการ '.$dept_name.' ตั้งแต่วันที่ '.getFirstRow($otdata_id,$year_id,$otdata_month,$conn).' ถึงวันที่ '.getLastRow($otdata_id,$year_id,$otdata_month,$conn).' </h4>    
    ';     
    ...
    <thead>  
    <tr>         
        <th width="10%" align="center" bgcolor="#c9c9c9"><b>ลำดับ</b></th>    
        <th width="25%" align="center" bgcolor="#c9c9c9"><b>รายชื่อ</b></th>        
        <th width="12%" align="center" bgcolor="#c9c9c9"><b>วัน/เดือน/ปี</b></th>        
        <th width="12%" align="center" bgcolor="#c9c9c9"><b>เวลาเริ่ม</b></th>
        <th width="12%" align="center" bgcolor="#c9c9c9"><b>เวลากลับ</b></th>
        <th width="18%" align="center" bgcolor="#c9c9c9"><b>ลายเซ็นผู้ปฏิบัติงาน</b></th>
        <th width="12%" align="center" bgcolor="#c9c9c9"><b>หมายเหตุ</b></th>   
    </tr>
    </thead>
    <tbody>      
    ';
    ...
    <table border="0" cellspacing="0" cellpadding="3">
        <tr>
            ...
            <td width="30%" align="center"> ____________________________  </td>
            <td width="20%">ผู้รับรองการปฏิบัติงาน</td>                       
        </tr>                     
    </table>
    ';
